...
export default
    {
        ...
        methods:
        {
                   ...
                   // if (event.data.length <= 6) {
                        ...
                        if (!util.validateRegex(charValue, /^[A-Za-z0-9\s]+$/)) {
                            //event.target.value = event.target.value.replace(charValue, "");
                                ...
                                if(charValue === '-' && this.contarGuiones < 1){
                                    this.contarGuiones = this.contarGuiones + 1;
                                }else if(charValue){
                                    event.target.value = event.target.value.replace(charValue, "");
                                }
            ...
            //GNM+{
                filtrar_autos_placa: function () {
                    this.popup_filtro_autos = true;
                },
                onpressBuscar : function(){
                    this.guillermo = this.searchPlaca; 
                    console.log(this.searchPlaca );
                    this.popup_filtro_autos = false;      
                },
                clear_autos_placa: function () {
                    this.searchPlaca = '';
                    this.guillermo = '';
                    this.popup_filtro_autos = false;
                }
        ...
        created() {
            ...
            this.isProvisional = this.$route.params.isProvisional;
            //05-06-2019
            ...
            if (this.cita) {
                ...
                this.codFamiliaVehiculo = this.cita.sCodFamiliaVehiculo;
            ...
            this.listaVehiculos = calendarioModel.listar_vehiculos(this);
        ...
        computed : {
            ...
            filteredItems: function() {
                   //if (this.placa) {
                    //return this.placa.filter((item) => item.zPlaca.includes(this.guillermo));
                //}
                var placaBuscar = this.guillermo;
                if(placaBuscar != ""){
                    placaBuscar =  placaBuscar.toUpperCase();
                }
                if(this.listaVehiculos != undefined){
                    // return this.listaVehiculos.filter( item.zPlaca.includes(placaBuscar)
                      if( this.listaVehiculos.length > 0){
                                return this.listaVehiculos.filter(function (item) {
                        if(item.zPlaca !== undefined && item.zPlaca !== null ){
                            if(item.zPlaca.includes(placaBuscar)){
                                return true;
                            }else{
                                return false;
                            }
                    ...
                    return this.$route.params.listaVehiculos;
                }
